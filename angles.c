...
** Last update Sat Feb 11 02:53:24 2017 Nicolas Polomack
*/
...
float		get_cos_angle_c(float dist, t_params *params, sfVector2i idxs)
{
  ...
  dir.y = params->light[idxs.y].pos.y - imp.y;
  dir.z = params->light[idxs.y].pos.z - imp.z;
  get_cylinder_normal(params, idxs, &imp, dist);
  add_coords_vect(&params->ray.orig, &params->ray.dir,
		  &(params->objs[idxs.x]));
  add_coords_vect(&params->light[idxs.y].pos, NULL,
		  &(params->objs[idxs.x]));
...
float		get_cos_angle_o(float dist, t_params *params, sfVector2i idxs)
{
  sfVector3f	imp;
  sfVector3f	dir;
  ...
  dir.y = params->light[idxs.y].pos.y - imp.y;
  dir.z = params->light[idxs.y].pos.z - imp.z;
  get_cone_normal(params, idxs, &imp, dist);
  add_coords_vect(&params->ray.orig, &params->ray.dir,
		  &(params->objs[idxs.x]));
  add_coords_vect(&params->light[idxs.y].pos, NULL,
		  &(params->objs[idxs.x]));
