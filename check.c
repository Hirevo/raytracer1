...
** Last update Thu Feb  9 02:48:05 2017 Nicolas Polomack
*/
...
int	check_sections(char type, int sec, int *lights, int *eyes)
{
...
int	is_valid_line(char *line, int *lights, int *eyes)
{
  int	len;
  int	idx;
  int	sec;
  ...
  return (check_sections(line[0], sec, lights, eyes) ? 1 : 0);
}
