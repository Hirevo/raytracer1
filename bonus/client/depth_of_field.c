...
** Last update Fri Mar 17 12:06:44 2017 Nicolas Polomack
*/
...
void	set_focal_dist(t_thread *t)
{
  ...
  t->ray.dir = calc_dir_vector(t->params->screen_size,
                               t->params->screen_size.x / 2 + 100,
                               t->params->screen_size.y - 100,
                               t->params->fov);
  rotation_t_eye(t);
  i = -1;
...
sfColor		dof(t_thread *t, float x, float y)
{
  ...
  col = aver_col(t->depth_col, t->params->depth_rays *
		 t->params->depth_rays);
  return (col);
}
